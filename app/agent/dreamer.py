...
class DreamerAgent:
    ...
    async def _run_research_phase(self, state: ProjectState) -> "dict[str, str]":
        ...
        for ft in state.needs_research:
            question = (
                f"Task '{ft.title}' (stage={ft.stage!r}) is stalled in the Maestro "
                f"pipeline.  Most recent failure: {ft.failure_reason}\n\n"
                f"Task description: {ft.description[:300]}\n\n"
    ...
    async def _decide(
        ...
        for ft in actionable:
            ...
            lines = [
                ...
                f"  Title: {ft.title}",
                f"  Description: {ft.description[:200]}",
                f"  Failure: {ft.failure_reason}",
            ]
            if ft.task_id in rf:
                # Cap findings at 600 chars to keep the prompt manageable
                lines.append(f"  Research findings: {rf[ft.task_id][:600]}")
    ...
    async def _decide_survey(self, state: ProjectState) -> DreamerPlan:
        ...
        if state.deleted_tasks:
            ...
            for t in state.deleted_tasks:
                lines.append(
                    f"  - [{t['stage']}] {t['title']}: "
                    f"{t['description'][:150]} (last outcome: {t['outcome']})"
                )
            deleted_block = (
