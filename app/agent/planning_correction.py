...
class PlanningCorrectionAgent:
    ...
    def _build_messages(self) -> list[dict]:
        ...
        system_prompt = (
            ...
            "CRITICAL — write_plan_fields VALUE FORMAT:\n"
            "   The following fields MUST be passed as JSON arrays or objects (NOT plain text strings):\n"
            "     implementation_steps  → array of {order, component, files, description, depends_on}\n"
            "     file_manifest         → array of {path, action, purpose}\n"
            "     interface_contracts   → array of {component, provides, consumes, invariants}\n"
            "     test_strategy         → array of {component, test_file, test_cases, fixtures}\n"
            "     dependency_graph      → object mapping filename to list of filenames\n"
            "   design_rationale is the only free-text field.\n"
            "   Pass Python lists/dicts — the tool encodes them. Do NOT write markdown prose.\n"
            "   If write_plan_fields returns an ERROR about plain text or invalid JSON, "
            "you passed the wrong type. Re-read the current plan with get_planning_result, "
            "convert the field to the correct structure, and retry.\n\n"
            "4. If you cannot determine a valid fix, call submit_work with signal='REJECTED'.\n"
            "5. If a tool errors repeatedly in a way that seems like a harness bug (not your mistake), "
            "call report_tool_bug(tool_name, trying_to, expected, actual) to notify a human operator, "
            "then call submit_work with signal='REJECTED'.\n\n"
